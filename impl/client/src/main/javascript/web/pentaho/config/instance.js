...
define([
  ...
  /**
   * An AMD loader plugin for loading the configuration of instances.
   *
   * This plugin queries the [Configuration Service]{@link pentaho.config.IService}
   * for the configuration of a given instance, whose id is given as the plugin's argument.
   *
   * **AMD Plugin Usage**: `"pentaho/config/instance!{instance-id}"`
   *
   * 1. `{instance-id}` — the identifier of the instance whose configuration is desired;
   ...
   * **Example**
   *
   * The following module is an instance module and requests its own configuration:
   *
   * ```js
   * define(["pentaho/config/instance!"], function(myConfig) {
   *
   *   if(myConfig !== null) {
   *     // ...
   *   }
   ...
   * @amd pentaho/config/instance
   *
   * @see pentaho.config.type
