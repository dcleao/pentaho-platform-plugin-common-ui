...
define([
  ...
  /* eslint valid-jsdoc: 0, dot-notation: 0 */
  ...
  var Type = Base.extend("pentaho.type.Type", /** @lends pentaho.type.Type# */{
    ...
    constructor: function(spec, keyArgs) {
      ...
      // Using mix records the applied instSpec, while #extend does not.
      this.constructor.mix(spec, null, keyArgs);
    ...
    set mixins(values) {
      ...
      var InstCtor = this.instance.constructor;
      ...
      function addMixinType(MixinType) {
        var MixinInstCtor = baseLoader.getType(MixinType);
        InstCtor.mix(MixinInstCtor);
      }
    },
    // endregion
    ...
    */
     ...
     *
     * This method delegates to [ILoader#get]{@link pentaho.type.ILoader#get}
     ...
     * </caption>
     *
     * require(["pentaho/type/Value"], function(Value) {
     *
     ...
     *
     *   var ProductList = loader.getType([{
     *     props: [
     *       "id",
     *       "name",
    ...
    create: function(instRef, instKeyArgs) {
      return baseLoader.getInstance(instRef, instKeyArgs, this);
    },
