...
define([
  ...
  "pentaho/i18n!types",
  "pentaho/config!"
], function(module, Type, Element, typeUtil, O, error, F, bundle, config) {
    ...
    toSpecInContext: function(keyArgs) {
      ...
      if(!includeType && declaredType) {
        ...
        if(type !== declaredType) {
          if(!stringType) {
            stringType = Type.get("string").type;
            numberType = Type.get("number").type;
            booleanType = Type.get("boolean").type;
          }
      ...
      if(!objectType) objectType = Type.get("object").type;
  ...
  .implement({$type: bundle.structured.simple})
