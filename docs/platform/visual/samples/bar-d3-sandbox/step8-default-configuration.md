---
title: Step 8 - Adding a default configuration
...
the developer of `V1` may package it with a configuration module so that it better integrates with `A1`, 
out-of-the-box.
...
define(["module"], function(module) {
  ...
  var vizModelId = module.id.replace(/(\w+)$/, "model");
...
For such, edit the `package.json` file and add 
a [`pentaho/instanceInfo`]({{site.refDocsUrlPattern | replace: '$', 'pentaho.instanceInfo'}})
